@extends('layouts.app')
@section('content')

<div class="jumbotron" id="servicios-jumbotron">
	<div class="container">
		<h1><b>Desarrollo de aplicaciones</b></h1>

		<div class="row">
			<div class="col-xs-12 col-md-12">
				<img class="img-fluid" src="images/AppsAla Medida.jpg" />
			</div>

			<div class="col-md-12 col-xs-12">

				<div style="margin:15px 15px 15px 15px;">
					<h3>Dando vida a tus proyectos.</h3>
					<p class="text-justify">
						Tras horas y horas de arduo trabajo, de seguro en más de alguna oportunidad, has fantaseado con lo fácil que sería, si esa labor se realizara de manera automatizada, por un programa en tu ordenador o por una aplicación móvil en tu teléfono.
						En mAppu te invitamos a soñar y fantasear con todas aquellas grandes ideas, que harán que tu negocio despegue. cuéntanos tus proyectos y nosotros a través de nuestro trabajo le daremos vida.


					</p>
					<center>
						<!-- <img class="img-fluid" src="images/Aplicaciones-mAppu.png" alt="imagen asesoria"> -->

					</center>

				</div>
				<div style="margin:15px 15px 15px 15px;">
					<h3>¿Por qué una Aplicación?</h3>
					<p class="text-justify">
						Las aplicaciones o Apps son herramientas informáticas desarrolladas para ayudar a los usuarios a realizar labores o tareas que en muchos casos resultas ser complejas, tediosas o requieren de mucho tiempo. Por lo tanto, una aplicación en tu negocio:

					</p>
					<center>
						<!-- <img class="img-fluid" src="images/Aplicaciones-mAppu.png" alt="imagen asesoria"> -->

					</center>
					<br />
					<div class="row">
						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>

								<p>
									<b>Disminuye tiempos.</b>
								</p>
								<img src="images/Tiempos.png" alt="" width="150px" height="150px">
							</center>
						</div>

						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>
								<p>
									<b>Simplifica procesos.</b>
								</p>
								<img src="images/proceso.png" alt="" width="150px" height="150px">
							</center>
						</div>

						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>
								<p>
									<b>Generan registros.</b>
								</p>
								<img src="images/Genera registros.png" alt="" width="150px" height="150px">
							</center>
						</div>
					</div>
					<br>
					<div class="row">
						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>
								<p>
									<b>Aumenta la comunicación.</b>
								</p>
								<img src="images/comunicacion.jpg" alt="" width="150px" height="150px">
							</center>
						</div>

						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>
								<p>
									<b>Genera accesibilidad.</b>
								</p>
								<img src="images/accesibilidad.png" alt="" width="200px" height="150px">
							</center>
						</div>

						<div class="col-xs-12 col-md-4 col-xl-4">
							<center>
								<p>
									<b>Apoya el control.</b>
								</p>
								<img src="images/control.png" alt="" width="200px" height="150px">
							</center>
						</div>
					</div>

				</div>
			</div>


		</div>
		<br />
		<br />

		<h1 class="text-left">Aplicaciones móviles Android</h1>
		<br />
		<br />
		<div class="row">
			<div class="col-xs-12 col-md-4 col-xl-4">
				<center>
					<img src="images/android_logo_PNG18.png" alt="">
				</center>
			</div>
			<div class="col-xs-12 col-md-8 col-lg-8">
				<br>
				<p class="justify">Las aplicaciones móviles Android, son herramientas desarrolladas para ser ejecutadas en Smartphone o Tablet con sistema operativa Android, logrando así cabal uso de las funcionabilidades de tu deposito móvil.</p>
			</div>
		</div>


		<br />
		<br />

		<h1>Aplicaciones WEB</h1>
		<br />
		<br />
		<div class="row">
			<div class="col-xs-12 col-md-8 col-xl-8">
				<br>
				<p class="justify">Las aplicaciones web son herramientas informáticas que se alojas en sitios web, permitiendo su acceso desde cualquier dispositivo, como notebook, Tablet o Smartphone que cuente con un navegador de internet. </p>
			</div>
			<div class="col-xs-12 col-md-4 col-lg-4">
				<img src="images/App-web.png" alt="">
			</div>
		</div>

		<br />
		<br />

		<h1>Aplicaciones móviles multiplataforma</h1>
		<br />
		<br />
		<div class="row">
			<div class="col-xs-12 col-md-4 col-lg-4">
				<img src="images/app-hibrida.png" alt="">
			</div>
			<div class="col-xs-12 col-md-8 col-xl-8">
				<br>
				<p class="justify">Las aplicaciones móviles multiplataforma o hibridas, son herramientas informáticas, destinadas a ser utilizadas en dispositivos móviles, sin importar el sistema operativo con que estos cuenten.</p>
			</div>
		</div>

			<br>
			<br>
		<div class="publicidad">
			<div class="row">
				<div class="col-md-6">
					<h4>Tú podrias potenciar tu negocio</h4>
					<p>Atrévete a innovar con nosotros</p>
				</div>

				<div class="col-md-6">
					<form>
						<label>¿Deseas estar al tanto de nuestras novedades?</label>
						<div class="form-row align-items-right">
							<div class="row flex-items-xs-middle flex-items-xs-between" align="right">
								<div class="col-md-7">
									<input type="text" class="form-control" name="email" placeholder="mail@ejemplo.com">
								</div>
								<div class="col-md-5">
									<input type="submit" class="form-control" value="Suscribirme">
								</div>
							</div>
						</div>
					</form>
				</div>
			</div>
		</div>
			<br>
		<div class="contactanos" align="right">
			<a href="{{route('contacto')}}"><button type="button" class="form-control">Contáctanos</button></a>
		</div>


	</div>
</div>

@include('modals.clienteLayout')
@endsection
